...
def get_in_vivo_base_files(filter_date = False, test_run = False):
    files = [os.path.join(root, name)
             for root, dirs, files in os.walk(root_data_dir)
             for name in files
             if name.endswith(".xlsx") and 'Rep' in root and 'In vivo' in root]
    if files == []:
...
def archive_in_vivo_absorption():
    in_vivo_base_files = get_in_vivo_base_files()
    df_cols = ['Wavelength (nm)', 'Tech_rep_1', 'Tech_rep_2', 'Tech_rep_3']
    for file in in_vivo_base_files:
        # noinspection PyTypeChecker
        fname = file.split('/')[-1].split('.')[0]
        data_fpath = f"{archiving_folder}/In_vivo_Absorption/{fname}.parquet"
        metadata_fpath = f"{archiving_folder}/In_vivo_Absorption/{fname}_metadata.json"
        # if os.path.exists(data_fpath) and os.path.exists(metadata_fpath):
        #     continue
        try:
        ...
        with open(metadata_fpath, "w") as fp:
            ...
            # cols = ['Specie', experiment, 'Rep'] + [str(i) for i in wls]
        # spec_df = pd.DataFrame(columns = cols)
        # count = 0
        # for specie in species:
        #     subfiles = [file for file in  if specie in file]
        #     for file in subfiles:
        # temp = file.split(unit_str)[0].split('/')[-1]
        # rep = file.split('Rep')[1].replace(' ', '')[0]

        # spectra = np.transpose(np.array(df.iloc[:, 1:]))
