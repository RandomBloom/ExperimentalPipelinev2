...
def get_concentrate_base_files(filter_date = False, test_run = False):
    files = [os.path.join(root, name)
             for root, dirs, files in os.walk(root_data_dir)
             for name in files
             if name.endswith(".xlsx") and 'Rep' in root and 'Concentrate' in root]
    if files == []:
...
def archive_concentrate_absorption():
    concentrate_base_files = get_concentrate_base_files()
    df_cols = ['Wavelength (nm)', 'Tech_rep_1', 'Tech_rep_2', 'Tech_rep_3']
    for file in concentrate_base_files:
        # noinspection PyTypeChecker
        fname = file.split('/')[-1].split('.')[0]
        data_fpath = f"{archiving_folder}/Absorption_Concentrate/{fname}.parquet"
        metadata_fpath = f"{archiving_folder}/Absorption_Concentrate/{fname}_metadata.json"
        try:
